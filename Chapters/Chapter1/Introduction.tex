...
\begin{itemize}
	...
	\item Beacons or receivers, placed around the area the lawn mower will move in. Depending on the terrain, anywhere from 2 \todo{is two enough? Answer: It is according to the GoT site (see reference in LaTeX comments)} to more than 20 of these can be used: The more is placed, the more accuracy can be obtained to fight against any ambient noise.
...
\noindent
For instance, the autonomy of the vehicle (both in energy and for the navigation), and the overall cost should be considered \todo{insert price approximation here}. Even though the GoT system itself has a cost beyond anything a normal customer would pay for a lawn mower, it appears, at first, as a good solution for us in terms of accuracy and energy consumption compared to GPS-like systems which are also quite expensive \todo{insert price approximation here - not so true actually}. \\

\noindent
For an improvement, we could consider replacing it with a similar solution as it is only a simple brick of the whole system. \todo{This sentence should be perhaps moved to a dedicated part of the report}\\
