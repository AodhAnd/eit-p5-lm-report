...
An expression for the current, $i_a$, is required to derive a model for the electrical system. In \figref{fig:MotorElectric} an electrical diagram of the motor is displayed.
...
\begin{flalign}\centering
\omega_m(s) = \frac{\tau_m(s)}{J_m(s) \cdot s+B}
...
\begin{flalign}\centering
Closed\,loop = \frac{Y(s)}{R(s)} = \frac{Direct\,term}{1 + Open\, loop}
\label{eq:closedLoopMotor}
\end{flalign}

\begin{flalign}\centering
\frac{V_m(s)}{\tau_m(s)} = \frac{\frac{K_t}{s \cdot L_m + R_m}}{\frac{K_e}{J_m \cdot s + B}}
\label{eq:motortransferfunction}
\end{flalign}

\begin{flalign}\centering
\frac{V_m(s)}{\tau_m(s)} = \frac{K_t \cdot (J_m \cdot s + B)}{K_e \cdot (L_m \cdot s + R_m)}
\label{eq:motortransferfunction}
\end{flalign}
