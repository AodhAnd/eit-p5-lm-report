...
\begin{flalign}\centering
\eqref{eq:BlackBoxGearNewtonSecLawLaplace} \xRightarrow{} N_d \cdot F_c(s) = J_d \cdot s \cdot \Omega_d(s) + N_d \cdot F_b(s) \xRightarrow{} F_c(s) =  \frac{J_d \cdot s \cdot \Omega_d(s) + N_d \cdot F_b(s)}{N_d} \xRightarrow{} F_c(s) =  \frac{J_d \cdot s}{N_d} \cdot \Omega_d(s) + F_b(s)
...
\begin{flalign}\centering
\omega_d = \frac{N_m}{N_d} \cdot \omega_m \xRightarrow{\mathcal{L}} \Omega_d(s) = \frac{N_m}{N_d} \cdot \Omega_m(s).
\label{eq:BlackBoxGearNewtonSecLaw}
\end{flalign}
%
The same principle is applied from the angular velocity $\Omega_d$ to the translational velocity $V$:
\begin{flalign}\centering
v = r_t \cdot \omega_d = r_t \cdot \frac{N_m}{N_d} \cdot \omega_m \xRightarrow{\mathcal{L}} V(s) = \frac{r_t \cdot N_m}{N_d} \cdot \Omega_m(s).
\label{eq:BlackBoxGearNewtonSecLaw}
\end{flalign}

It is now possible to replace $\Omega_d$ and $F_b$ in \eqref{eq:GearsContactForceLaplace}:
\begin{flalign}\centering
\eqref{eq:GearsContactForceLaplace} \xRightarrow{} F_c(s) =  \frac{J_d \cdot s}{N_d} \cdot \frac{N_m}{N_d} \cdot \Omega_m(s) + V(s) \cdot \left[\frac{M \cdot s - B_{sys}}{r_t}\right] = \frac{J_d \cdot N_m \cdot s}{N_d^2} \cdot \Omega_m(s) + \frac{N_m}{N_d} \cdot \Omega_m(s) \cdot \left[M \cdot s - B_{sys}\right].
\label{eq:GearsContactForceLaplace}
\end{flalign}
