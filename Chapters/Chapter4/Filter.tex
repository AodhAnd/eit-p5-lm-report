...
There is many pros and cons when deciding between implementing an analogue or digital filter. But since the signal received from the magnetometer is digitalized, it has been chosen to implement a digital filter.
...
\end{flalign}
%
The Magnetometer thus has to be sampled at atleast every 106 \si{Hz}. 
...
With this in consideration, it has been decided to have a sampling rate of 66,66 \si{Hz}, i.e. a sample every 15 \si{ms}, even though it will cause a delay. The 66,66 \si{Hz} has been chosen instead of the 75 \si{Hz} to have a margin from the maximum sampling rate the sensor can deliver.
...
\begin{figure}[H]
  \centering
 	%Trim margins @:   left        bottom       right       top
 	\adjustbox{ trim = {.15\width} {.32\height} {.15\width} {.30\height}, clip }
  {
    \includegraphics[width=1.4\textwidth]{figures/SimulatedStepResponsewithandwithoutFilter.pdf}
  }
...
\begin{figure}[H]
  \centering
 	%Trim margins @:   left        bottom       right       top
 	\adjustbox{ trim = {.15\width} {.34\height} {.15\width} {.30\height}, clip }
  {
