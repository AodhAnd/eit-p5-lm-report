...
\end{flalign}
%
The poles of the system are in 33.3 and 0 \si{\frac{rad}{s}}. When designing a filter it is necessary to consider the placement of the pole which is at the highest frequency. In this situation the pole which is at the highest frequency is the pole placed at 33.3 \si{rad/s}. A rule of thumb is to place the cut-off frequency of the filter atleast one decade after the highest frequency pole, i.e. 333 \si{\frac{rad}{s}}. If the cut-off frequency of the filter is beneath 333 \si{\frac{rad}{s}} it will influence the system's phase-margin, which can cause instability. 

By being aware of this requirement, it is possible to find a sampling frequency which can grant a cut-off frequency above 333 \si{\frac{rad}{s}}. The least required cut-off frequency of 333 \si{\frac{rad}{s}} is converted to \si{Hz}:
%
\begin{flalign}
...
In the specification for the Magnetometer \cite{HMC5883L}, the maximum sampling rate is stated to be 75 \si{Hz}. A sampling rate of 106 \si{Hz} was desired, since it at this rate would not effect the system. 
...
There is different methods for transferring a continuous-time filter to a discrete-time filter, the two most common transformation methods are examined. The first method of the two is the impulse invariance transformation.
...
\end{flalign}
%
The poles placement indicated that it is two complex pole-pairs, since they are symmetric around the real-axis.
...
\end{figure}
%
The discrete specifications pre-warped to continuous-time, see \eqref{eq:preqwardspecifications}, is determined for the passband frequency to 166 \si{Hz} and for the stopband to \si{644}. Furthermore from the requirements set in \secref{sec:FilterRequirements}, the attenuation in the passband should be beneath 1 \si{dB} and the stopband should be at a attenuation of 40 \si{dB}. These requirements are in compliance with what is observed on \figref{fig:Continuoustimebodeplot}. Additionally, the cut-off frequency calculated in \eqref{eq:OrderandCutoffFrequencyCON} corresponds to what is illustrated in the frequency response.
