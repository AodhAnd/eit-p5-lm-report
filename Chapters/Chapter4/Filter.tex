...
0.89125^2 &= \frac{1}{1+(\frac{166}{\Omega_C})^{2N}} \quad \wedge \quad 0.01^2 = \frac{1}{1+(\frac{644}{\Omega_C})^{2N}}
 \end{flalign}
%
The cut-off frequency is calculated to 197.9 \si{Hz} and the order to 3.9, rounded to the nearest integer, i.e. 4. When the order is known it is possible to utilize the following equation for finding the poles for a Butterworth filter.
%
\begin{flalign}
