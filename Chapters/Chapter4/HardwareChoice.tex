...
\todo{References to the Storage section, Mads}
...
\end{minipage}
 ...
 \todo{Reference to the picture Mads}
...
\todo{There is a reference here which needs to be made, Mads}
...
The servomotor needs to receive commands every \si{30\ ms}, \cite{futaba}. The command therefore has to be ready in the meantime and is made from the angular sensor's data. It is chosen to have at least twice the servomotor's sampling frequency, for the angular sensor:

\begin{flalign}
	\eq{Sampling\ frequency}{2 \cdot \frac{1}{0.0030}  = 66.6 \si{Hz}}
\end{flalign}

The requirements for the angular sensor are:
\begin{itemize}
\item It is possible to control it by utilizing the Arduino Mega.
\item Able to achieve a sampling frequency at 66,6 \si{Hz}.
\end{itemize}

\subsubsection{HMC5883L}
Sparkfun's "9 degrees of freedom" board \todo{reference Mads til hele boardet!} comprises a magnetometer integrated circuit, the HMC5883L\cite{HMC5883L}. A magnetometer will be used as the angular sensor, since it can be set up as a compass and give out a angle compared to north. 
