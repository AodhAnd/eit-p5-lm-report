...
Timer 0 \todo{Needs to be streamlined: either each occurence is italic or none of them.} is reserved by the Arduino for it's internal time keeping functions, and not to be touched \cite{ArduinoPWM}. This leaves 5 timers, that can be used freely.

It would be advantageous to utilize the input capture\footnote{An input capture timer samples a free running counter, whenever triggered by  an external event} \todo{Why would it be advantageous?} functionality of the timers to read the hall sensors. After examining the Arduino Mega pinout (\secref{megaPinout}), it is seen that only Timer 4 and 5 have their input capture pins available on the board (ICP4 and ICP5). These are therefore chosen for the two hall sensors.
   
The real-time operating system \todo{We need to specify which at some point} needs a timer, for running its scheduler. Timer 1 is chosen for this, for no other reason than it being the default setting.
...
As the timers have now been designated, the next step is to assign the various serial ports. This is done as shown on \figref{MegaSetup}.\todo{Please precise also that the power line connections aren't drawn on that schematic}
...
A low-dropout type is chosen, L4941, to make sure it works, even at low battery voltages. This regulator has a maximum dropout of 700mV at 1A current, meaning that it will work with battery voltages as low as $\SI{5,7}{V}$. \todo{datasheet + Maybe add a bit more explanation and justification for these values} 
...
\begin{figure}[H]
	...
	\label{i2clevel}
\end{figure}
\todo{If we give that much explanations for a voltage divider, we also need to expand on this circuit!}
...
To make sure that this does not happen, the resistors in the voltage divider are set to half of the maximum output impedance, \si{5\ k\Omega}. The closest resistor in the E96 series\todo{Is it possible to explain that a bit further for non-tech readers?} is \si{4,99\ k\Omega}, which will be used for both resistors. 
...
\eq{\omega_c}{\frac{1}{5 \cdot 10^{3} \cdot 100 \cdot 10^{-9}}}\unit{rad \cdot s^{-1}} \nonumber \\
\eq{\omega_c}{2 \cdot 10^{3}}\si{\ rad \cdot s^{-1}}&\nonumber
\end{flalign}
