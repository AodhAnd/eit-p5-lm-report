...
The running time of this task is 386µs. As the speed control task has to run 2 times faster than the hall sensors task to get a new data every time, the minimum period will be 2ms.
...
The running time of the Distance Control is 153.6µs, and it runs just after the GoT System and Communication Protocol, so has a sampling time also of 100ms.

\textbf{Angular Control:}
The steering task gets the reading from the magnetometer, and transform them in the coordinate system that fit the model. Then those values are converted into a heading angle, that will be used in the steering P-Controller to calculate the new angle to follow from the angle wanted. The decision will be sent directly to the servo. This is the inner loop from the Steering Model, seen in \secref{sec:SteeringModel}.\\
...
The tasks ask a request of running every each period of time regarding their setup, the global view of these request can be seen in \figref{scheduleRequest}
 \begin{figure}[H]
	\centering
...
As seen on the  \figref{schedulePriorities}, the angular control has the highest priority so it will run first, and the others will be registrered in a queue in order of priority. Then the hall sensors are second, and speed control after that. When the the speed control will be running, it can can a second request to run because of it's sampling time. It will then run a second time just after that because it's priority is higher than the last task. The GoT and comunication will run after them, and in the end the Distance Control.\\
